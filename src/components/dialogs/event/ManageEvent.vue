...
// Dialog components - Fixed imports (these should be different components)
import EventFormDialog from "@/components/dialogs/forms/CreateBusinessEvent.vue";
import EventViewDialog from "@/views/event/EventViewer.vue"; // Different component
import EventDeleteDialog from "@/components/dialogs/event/ManageEvent.vue" // Different component
...
import ProGenerateVoucher from "@/components/dialogs/payments/ProGenerateVoucher.vue";
...
const editEvent = (event: EventType) => {
  ...
  dialogStore.open(DIALOGS.EVENT_FORM, { event });
};
...
const deleteEvent = (event: EventType) => {
  ...
  dialogStore.open(DIALOGS.EVENT_DELETE, { event });
};

function handleDialogClose(dialogName: string) {
  dialogStore.close(dialogName);
  if (dialogName === DIALOGS.EVENT_FORM) eventStore.setEditingEvent(null);
  if (dialogName === DIALOGS.EVENT_VIEW) eventStore.setViewingEvent(null);
  if (dialogName === DIALOGS.EVENT_DELETE) eventStore.setDeletingEvent(null);
}

async function handleFormSaved() {
  dialogStore.close(DIALOGS.EVENT_FORM);
  await userStore.refreshDashboard();
  await resetAllStores();
}

async function handleDeleteConfirmed() {
  dialogStore.close(DIALOGS.EVENT_DELETE);
  await userStore.refreshDashboard();
  await resetAllStores();
}
...
<template>
  <v-container fluid class="pa-sm-2 fill-width">
    ...
    <!-- Event Form Dialog -->
    <v-dialog
      :model-value="dialogStore.isDialogOpen(DIALOGS.EVENT_FORM)"
      max-width="800"
      persistent
      @update:model-value="(val) => !val && handleDialogClose(DIALOGS.EVENT_FORM)"
    >
      <EventFormDialog
        @saved="handleFormSaved"
        @cancel="handleDialogClose(DIALOGS.EVENT_FORM)"
      />
    </v-dialog>

    <!-- Event View Dialog -->
    <v-dialog
      :model-value="dialogStore.isDialogOpen(DIALOGS.EVENT_VIEW)"
      max-width="700"
      @update:model-value="(val) => !val && handleDialogClose(DIALOGS.EVENT_VIEW)"
    >
      <EventViewDialog @close="handleDialogClose(DIALOGS.EVENT_VIEW)" />
    </v-dialog>

    <!-- Event Delete Dialog -->
    <v-dialog
      :model-value="dialogStore.isDialogOpen(DIALOGS.EVENT_DELETE)"
      max-width="500"
      persistent
      @update:model-value="(val) => !val && handleDialogClose(DIALOGS.EVENT_DELETE)"
    >
      <EventDeleteDialog
        @confirmed="handleDeleteConfirmed"
        @cancel="handleDialogClose(DIALOGS.EVENT_DELETE)"
      />
    </v-dialog>
