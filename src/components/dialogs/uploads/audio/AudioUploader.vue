<template>
  <v-container>
    ...
    <div @dragover.prevent="dragover" @drop.prevent="drop" class="drop-zone">
      <v-file-input v-model="files" @change="validateFiles, handleFileChange"
        accept=".csv, .json, .xml, .parquet, .xlsx, .txt, .tsv, .sql, .pdf, image/*, video/*, audio/*"
        label="Upload Tutorial Resources" prepend-icon="mdi-cloud-upload" :multiple="true" :counter="true"
        :rules="fileRules" :chips="true" show-size></v-file-input>
      <p class="text-center">or drag and drop files here</p>
    </div>
    ...
    <!-- Upload Progress Bar -->
    <v-progress-linear v-if="uploading" :value="uploadProgress" height="6" color="green darken-2" class="mt-4">
    </v-progress-linear>
