<template>
  <v-container>
    <v-file-input v-model="files" accept=".jpg,.jpeg, .png" label="Upload a File" prepend-icon="mdi-file"
      :multiple="true" :counter="true" :rules="fileRules" :chips="true" @change="validateFiles"
    ...
    <v-progress-linear v-if="uploading" :value="uploadProgress" height="6" color="green darken-2"
      class="mt-4"></v-progress-linear>

    <div v-if="isUploadSuccessful" class="custom-alert">
      <v-alert :text="isUploadSuccessful" title="Files uploaded successfully!" type="success"
