...
// Correct dialog components
import EventFormDialog from "@/components/dialogs/forms/CreateBusinessEvent.vue";
import EventViewDialog from "@/components/dialogs/forms/CreateBusinessEvent.vue";
import EventDeleteDialog from "@/components/dialogs/forms/CreateBusinessEvent.vue";
import CreateOrganization from "@/components/dialogs/forms/CreateOrganization.vue";
import LegalDocs from "../dialogs/uploads/pdf/LegalDocs.vue";
...
// helper: checks if a row is the active one
const isActive = (item: any) => {
  if (!activeRow.value) return false;

  // support both id and _id cleanly
  const activeId = activeRow.value.id || activeRow.value._id;
...
// -------------------------------------------------------
// COMPUTED — SAFE EVENT LIST
// -------------------------------------------------------
onMounted(async () => {
  try {
    // Reset all stores first for clean state
    ...
    activeRow.value = eventStore.currentEvent; // Mark as managing
  } catch (err) {
    console.error("❌ Error in onMounted lifecycle:", err);
  }
});

import OrgDetails from "../dialogs/details/OrgDetails.vue";
import ProGenerateVoucher from "../dialogs/payments/ProGenerateVoucher.vue";
import ManageEvent from "../dialogs/event/ManageEvent.vue";
...
const setActiveEvent = async (eventRow: EventType) => {
  ...
  // Reset tool stores before switching to new event
  await resetAllStores();
...
const deleteEvent = (event: EventType) => {
  ...
  dialogStore.open("eventDelete");
};

function handleDialogClose(dialogName: string) {
  dialogStore.close();
  if (dialogName === "eventForm") eventStore.setEditingEvent(null);
  if (dialogName === "eventView") eventStore.setViewingEvent(null);
  if (dialogName === "eventDelete") eventStore.setDeletingEvent(null);
}

async function handleFormSaved() {
  dialogStore.close();
  await userStore.refreshDashboard();
  await resetAllStores(); // Reset stores after form save
}

async function handleDeleteConfirmed() {
  dialogStore.close();
  await userStore.refreshDashboard();
  await resetAllStores(); // Reset stores after delete
}

const activateOrg = async (org: any | null) => {
  if (!org) return;

  // Reset stores before activating new organization
  await resetAllStores();

  orgStore.currentOrganization = org; // Set current org in Pinia
  loadOrgEvents(org); // Load events for this org
};
...
watch(
  ...
  async (id) => {
    if (id) {
      await userStore.refreshDashboard();
      await resetAllStores(); // Reset stores when user changes
    }
  },
  { immediate: true },
);
...
watch(
  ...
  (isOpen) => {
    if (isOpen) {
      const dlg = dialogStore.currentDialog;
      if (dlg && dlg in dialogVisible) {
        dialogVisible[dlg as keyof typeof dialogVisible] = true;
      }
    } else {
      Object.keys(dialogVisible).forEach((k) => (dialogVisible[k] = false));
    }
  },
);

// -------------------------------------------------------
// EXPORT (safe placeholder)
// -------------------------------------------------------
...
const getVoucher = (org: any) => {
  orgStore.currentOrganization = org;
  orgStore.newVoucherDialog = true;
};
</script>
  ...
  <v-container fluid class="pa-sm-2 fill-width">
          ...
          <v-col cols="12" sm="4" class="text-center">
            ...
            >
              ...
              <div>
                ...
                <p class="text-grey mb-0 text-text-caption text-body-2-sm">
        ...
        <v-row dense>
          ...
          <v-col cols="12" sm="6" md="4" class="pb-2">
            <div class="d-flex flex-wrap gap-2 w-100">
              <v-btn
                variant="tonal"
                @click="orgStore.dialogOpen = true"
                size="small"
    ...
    <v-card class="mt-2">
      ...
      <v-data-table
        ...
        :item-class="(item, index) => getRowClass(item, index)"
        density="comfortable"
        item-value="id"
      >
        <!-- Loading State -->
    ...
    <!-- DIALOGS -->
    <v-dialog v-model="orgStore.dialogOpen" max-width="630" persistent>
      <CreateOrganization />
    </v-dialog>
    <v-dialog v-model="orgStore.createOrgUpload" max-width="630" persistent>
      <LegalDocs />
    </v-dialog>
    <v-dialog
      v-model="dialogVisible.eventForm"
      max-width="800"
      persistent
      @update:model-value="(val) => !val && handleDialogClose('eventForm')"
    >
      <EventFormDialog
        @saved="handleFormSaved"
        @cancel="handleDialogClose('eventForm')"
      />
    </v-dialog>

    <v-dialog
      v-model="dialogVisible.eventView"
      max-width="700"
      @update:model-value="(val) => !val && handleDialogClose('eventView')"
    >
      <EventViewDialog @close="handleDialogClose('eventView')" />
    </v-dialog>

    <v-dialog
      v-model="dialogVisible.eventDelete"
      max-width="500"
      persistent
      @update:model-value="(val) => !val && handleDialogClose('eventDelete')"
    >
      <EventDeleteDialog
        @confirmed="handleDeleteConfirmed"
        @cancel="handleDialogClose('eventDelete')"
      />
    </v-dialog>

    <v-dialog v-model="orgStore.dialogDetailOpen" max-width="630">
      <OrgDetails />
    </v-dialog>
    <v-dialog v-model="orgStore.showLegalUpload" width="630">
      <LegalDocs />
    </v-dialog>

    <v-dialog v-model="orgStore.newVoucherDialog" width="66rem">
      <ProGenerateVoucher />
    </v-dialog>
    <v-dialog v-model="eventStore.managingEvent" width="75%">
      <ManageEvent />
    </v-dialog>
  </v-container>
</template>
...
.selected-org {
  ...
  background-color: rgba(var(--v-theme-#dadada), 0.4);
}
...
.v-data-table tbody tr:hover {
  ...
  border-left: 4px solid #3cbac6 !important; /* left border */
  border-right: 4px solid #3cbac6 !important; /* right border */
}
