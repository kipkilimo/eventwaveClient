<template>
  <div>
    <!-- VSAQ Section -->
    <v-card class="vsaq-container" elevation="0">
      <v-card-title class="d-flex justify-space-between align-center">
        <span class="text-h5">SECTION B Part I - Very Short Answer Questions</span>
      ...
      <v-card-text>
        ...
        <v-card elevation="2">
          ...
          <v-card-text>
            <!-- Text Answer Input -->
            <v-textarea :disabled="responseType === 'file' || currentAnswer.submitted" v-model="currentAnswer.text"
...
const questions = computed(() => {
  return questionStore.allQuestions.filter(q => q.questionType === 'VERY_SHORT_ANSWER');
});
...
const submitCurrentQuestion = async () => {
  ...
  try {
    ...
    localStorage.setItem('vsaq_answers', JSON.stringify(userAnswers.value));
...
onMounted(() => {
  const savedAnswers = localStorage.getItem('vsaq_answers');
  if (savedAnswers) {
    try {
