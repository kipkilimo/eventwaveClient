<template>
  <div>
    <!-- VSAQ Section -->
    <v-card class="saq-container" elevation="0">
      <v-card-title class="d-flex justify-space-between align-center">
        <span class="text-h5">SECTION B Part II - Short Answer Questions</span>
...
const questions = computed(() => {
  return questionStore.allQuestions.filter(q => q.questionType === 'SHORT_ANSWER');
});
...
const submitCurrentQuestion = async () => {
  ...
  try {
    ...
    localStorage.setItem('saq_answers', JSON.stringify(userAnswers.value));
...
onMounted(() => {
  const savedAnswers = localStorage.getItem('saq_answers');
  if (savedAnswers) {
    try {
