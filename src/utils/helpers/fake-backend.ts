...
function fakeBackend() {
  const users: User[] = [{ id: 1, username: 'info@codedthemes.com', phone: 'admin123', firstName: 'Codedthemes', lastName: '.com' }];
  ...
  window.fetch = function (url: string, opts: { method: string; headers: { [key: string]: string }; body?: string }) {
    return new Promise<Response>((resolve, reject) => {
      ...
      function authenticate() {
        ...
        const user = users.find((x) => x.username === username && x.phone === phone);
        if (!user) return error('Username or phone is incorrect');
        return ok({
          id: user.id,
